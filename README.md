...
| `goals/goal.service.ts` (+ Pg store) | Phase 3 of the GrowthOS-aligned restructuring plan — turns a tenant's stated ambition into a real, measurable Goal (metric/baseline/target/deadline) instead of the one free-text field that existed before, with computed progress % and an auto-flagged `at_risk` status when progress falls behind schedule | **17/17 tests pass**, including that `computeAutoStatus()` never overrides a human/system-set `achieved`/`abandoned` status, and that reads apply the recomputed status live without persisting it. |
| `growth-actions/growth-action.service.ts` (+ Pg store) | Phase 4 — a real, stateful task (todo → in_progress → done/dismissed), distinct from the dashboard's own derived "today's priorities"; `relatedGoalId` is auto-set when converting a Trigger whose business area matches an open Goal, closing the one link the Trigger→Goal automation loop was missing | **12/12 tests pass**, including the real auto-link to a matching Goal. |
| `triggers/trigger.service.ts` (+ Pg store) | Phase 2 — persists the trigger-detection logic that used to live only inside `automation.service.ts`'s `notificationsFor*()` functions as a real, dismissible dashboard row instead of a fire-and-forget notification | **8/8 tests pass**. |
| `crm/crm.service.ts` + `crm-stale-lead-check.service.ts` (+ Pg stores) | Phase 5 — the pre-sale lead/pipeline layer this codebase had none of before (Deals is a discount/promotion catalog, Customers is post-sale record-keeping); a won lead converts to a real Customer, and a daily job flags a lead gone stale | **16/16 tests pass**. |
| `retention/retention.service.ts` | Phase 6 — deliberately read-only, no new store: every number (repeat customers, churned/at-risk customers) is derived live from existing Sale/Customer data | **7/7 tests pass**. |
| `commission/commission.service.ts` + `staff-performance.service.ts` (+ Pg store) | The tenant's own explicit request (2026-09-15) — one active commission rate per staff member, composed with `SaleService`'s per-staff figures into a real performance view (sales, customers created/updated, commission earned) | **12/12 tests pass**. |
| `campaigns/campaign.service.ts` | The tenant's own explicit request (2026-09-14) — a named, multi-channel (Facebook/WhatsApp/Instagram/Website) promotional push, optionally reusing an existing Deal's own content instead of a second content model | **10/10 tests pass**. |
| `analytics/website-visit.service.ts` (+ Pg store) | A real gap the tenant asked about directly (2026-09-11) — a first-party website-visit collector for a tenant with no analytics of their own already set up; no raw IP addresses or user-agent strings stored anywhere, per the standing privacy constraint | **11/11 in-memory + 2/2 real-database tests pass**. |
| `quotations/quotation.service.ts` + `quotation-stale-check.service.ts` (+ Pg store) | The tenant's own explicit request (2026-09-16) — a real, editable draft document sendable by email/WhatsApp, a daily stale-quotation trigger (the same daily-job pattern already proven twice), and a one-click conversion of an accepted quote into a real Sale | **33/33 tests pass**. |
| `billing/subscription.service.ts` + `subscription-billing-check.service.ts` (+ Pg store) | Mytrima's own recurring ZAR subscription fee for Pro Plus/Growth Plan/Growth Partner, following the real prices published on the landing site's own Packages page, billed via a fresh MoPay checkout session each period since MoPay's documented API has no true auto-charge/subscription concept — disclosed, not hidden | **16/16 tests pass**, live-verified against the real MoPay sandbox. |
| `admin-auth/admin-auth.service.ts` (+ Pg stores) | Real per-admin login + mandatory MFA, replacing the single shared `ADMIN_API_KEY` guard with genuine per-operator identity — every admin JWT carries its own `admin_access`/`admin_refresh` type discriminator, making a tenant token structurally impossible to replay against an admin route or vice versa | **25/25 tests pass**, live-verified end-to-end: bootstrap admin registration, MFA enrollment (via a hand-rolled TOTP script), login, and both a real tenant access token and the old shared key correctly rejected on a migrated admin route. |
| `admin/admin-tenant.service.ts` + `platform-health.service.ts` | Real cross-tenant tenant management and platform performance for the operator — one combined per-tenant view (staff, tickets, subscription state, growth-audit/NPS/onboarding signals) reusing every existing signal rather than recomputing it; suspend/reactivate wired to the real, previously-unused `tenant.status` column (present since migration 0001); a manual subscription tier/status/next-billing-date override for a deal closed by phone or a subscription stuck in a bad state, bypassing self-service MoPay checkout; and real, honestly-gated platform metrics (MRR, past-due tenant count, signups by day, support-ticket resolution time, and each of the 6 real BullMQ queues reporting itself "not configured" rather than guessing when Redis/Postgres aren't set up) | **11/11 + 1 real-Postgres-gated test pass** (12 total). Live-verified end-to-end via curl and the running admin app: suspending a real tenant correctly blocked its owner's login with `TenantSuspendedError` (ahead of every other login check), reactivating restored it, an admin override wrote through and was confirmed on re-read, downgrading to "free" correctly cleared the billing date, and every platform-health field rendered its honest "not configured" state with no console errors. |

**780/780 tests pass with neither Postgres nor Redis available** (867 defined in total —
the remaining 87 need one or both and skip gracefully without them, see "Real
Postgres-backed stores" below). Run `npm test` to reproduce this yourself — don't take the
count on faith; this total was actually re-run, not carried forward from an earlier pass,
as of the admin-platform work above (2026-09-17). The gated tests need
`TEST_DATABASE_URL`/`TEST_REDIS_URL` set to a real local instance to run instead of skip —
see "Running this yourself" below. (One caveat worth naming: under heavy parallel test
load, a couple of the slowest multi-step real-database tests can occasionally exceed
Jest's default 5-second timeout — genuine timing tightness under contention, not a logic
bug; use `--maxWorkers=4` for exactly this reason.)
