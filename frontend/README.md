...
- **Snapshot** (`src/pages/SnapshotPage.tsx`) — the Business Snapshot report (`GET /reports/:tenantId/snapshot`): executive summary, period stat tiles, a real 30-day sales graph and per-product contribution, real-time daily monitoring (Budget/Actual/Same-day-last-year plus an hourly bar chart — see the main README's own "P.O.S., real refunds/exchanges..." section for the full detail), growth audit, findings, and recommended next steps.
...
- **P.O.S.** (`src/pages/POSPage.tsx`, renamed from "Sales" at the tenant's own request) — paginated sale list (`GET /sales/:tenantId?limit=&offset=`) and record-a-sale, with an optional live Deal picker; real refund/exchange processing per sale (`POST /sales/:tenantId/:saleId/refund` — see the main README's own top comment on why "exchange" is a refund plus an ordinary new sale, not a separate concept); and a "Petty cash" tab — a real, already-working backend module (`petty-cash.service.ts`/`vendor.service.ts`) that had no frontend anywhere before this: replenish the float, pay a vendor (creating one inline), and a real running-balance ledger.
- **Customers** — search, list, and add.
- **Catalog** — list, add, activate/deactivate, and a real per-item photo upload (click the thumbnail/placeholder).
- **Deals & promotions** (`src/pages/DealsPage.tsx`) — list + create (percentage off / buy-X-get-Y-free / fixed amount off, linked to real catalog items, with real start/end dates) and a real per-deal ad-image upload. Status shown is a real, computed Upcoming/Live/Ongoing/Expired from the deal's own dates — DealsController has no update/deactivate route today, and nothing anywhere counts real redemptions, so "monitor" here is honestly scoped to what's actually real right now, not a fabricated usage metric.
