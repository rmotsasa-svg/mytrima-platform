...
- Deals, Petty Cash, Growth Audit, NPS/Rating, Onboarding, Payments, and Social Publishing have real, gated backend endpoints already (see the main README) but no page here yet — this SPA covers the modules the tenant asked for by name (staff, booking, support) plus the core commerce loop (sales/customers/catalog) and the report that ties them together, not the full 19-module surface.
